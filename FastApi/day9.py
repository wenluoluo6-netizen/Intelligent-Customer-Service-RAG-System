...
# 定义依赖函数，用于获取Session的生成器函数，它的意思是：只要在 with 代码块里，session 就是打开的；一旦代码跑出去了，它会自动执行 session.close()，把窗口关掉。
def get_session():
...
async def read_all_todos(skip: int = 0, limit: int = 10, q: str | None = None, status: bool | None = None, session: Session = Depends(get_session)):
    # 1. 初始化查询语句：SELECT * FROM todo
    statement = select(Todo)
    # 2. 动态拼装查询条件
    if q:
        statement = statement.where(Todo.title.contains(q))
    if status is not None:
        statement = statement.where(Todo.is_completed == status)        # 这里两个statement会叠加存储
    # 3. 加上分页
...
async def update_todo(todo_id: int, todo_in: Todo, session: Session = Depends(get_session)):
    db_todo = session.get(Todo, todo_id)
    if not db_todo:
        raise HTTPException(status_code=404, detail="Todo not found")
    update_data = todo_in.dict(exclude_unset=True)      # exclude_unset=True 表示只更新需要更新的字段，保留其他字段不变
    for key, value in update_data.items():
        setattr(db_todo, key, value)        # Python 内置函数：给对象设置属性
    session.add(db_todo)
    session.commit()
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run("day9:app", host='127.0.0.1', port=8000, reload=True)